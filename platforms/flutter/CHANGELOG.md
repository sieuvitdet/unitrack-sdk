...
Cross-binary integration release. Khi 1 process iOS / Android host CẢ native
UniTrack SDK (qua SPM / Maven) lẫn Flutter plugin (qua pub.dev), giờ chỉ còn
1 singleton — cùng `session_id`, cùng SQLite, cùng provider list — thay vì 2
SDK song song không join được ở Portal.
