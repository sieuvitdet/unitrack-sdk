...
export function setMaxNodes(n: number) { maxNodes = n; }
 ...
 * to a flow layout.
 */
export function snapshotCurrentScreen(ref: any) {
...
function getFiberFromRef(ref: any): any {
  if (!ref) return null;
...
function walk(fiber: any, counter: { value: number }): WireframeNode | null {
  ...
  while (child) {
    if (counter.value >= maxNodes) break;
