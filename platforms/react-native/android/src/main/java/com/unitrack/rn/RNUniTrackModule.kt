...
class RNUniTrackModule(reactContext: ReactApplicationContext)
    ...
    @ReactMethod
    fun pendingProviderRetryCount(promise: Promise) {
        promise.resolve(UniTrack.pendingProviderRetryCount())
    }
    ...
    fun attachFirebaseAdapter(promise: Promise) {
        ...
        if (app != null) {
            ...
            // True if FirebaseAnalytics is on the classpath. JS side surfaces
            // a warning toast when host hasn't pulled in Firebase.
            promise.resolve(runCatching {
                Class.forName("com.google.firebase.analytics.FirebaseAnalytics"); true
            }.getOrDefault(false))
